...
public:
	...
	*/
	...
	\return Created flow.
	*/
	static flow_t *createFlow_t( const unsigned char proto, const unsigned char class_proto, 
								 const string& src_if, const string& dst_if, 
		const u_short src_port, const u_short dst_port, const unsigned int n_bytes, 
		const unsigned int n_frames, const time_t ini_sec, const time_t end_sec, const time_t ini_mic, 
		const time_t end_mic, in_addr ip_src, in_addr ip_dst);	 
