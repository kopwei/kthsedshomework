...
<%!String CardTypesAction(javax.servlet.http.HttpServletRequest request,
		...
		try {
			...
			if (iAction == iinsertAction || iAction == iupdateAction) {
				if (isEmpty(fldname)) {
					sCardTypesErr = sCardTypesErr
							+ "The value in field Name is required.<br>";
				}
			...
			case iinsertAction:
				...
				sSQL = "insert into card_types (" + "name)" +
			...
			case iupdateAction:
				...
				sSQL = "update card_types set " + "name="
	...
	void CardTypes_Show(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
			javax.servlet.jsp.JspWriter out, String sCardTypesErr,
		...
		try {
			...
			if (isEmpty(pcard_type_id)) {
				bPK = false;
			}

			sWhere += "card_type_id=" + toSQL(pcard_type_id, adNumber);
			primaryKeyParams += "<input type=\"hidden\" name=\"PK_card_type_id\" value=\""
					+ pcard_type_id + "\"/>";
			...
			if (bPK && !(sAction.equals("insert") && "CardTypes".equals(sForm))) {
				out
						.print("<input type=\"submit\" value=\"Update\" onclick=\"document.CardTypes.FormAction.value = 'update';\">");
				out
						.print("<input type=\"submit\" value=\"Delete\" onclick=\"document.CardTypes.FormAction.value = 'delete';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.CardTypes.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"CardTypes\"><input type=\"hidden\" value=\"update\" name=\"FormAction\">");
			}

			else {
				out
						.print("<input type=\"submit\" value=\"Insert\" onclick=\"document.CardTypes.FormAction.value = 'insert';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.CardTypes.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"CardTypes\"><input type=\"hidden\" value=\"insert\" name=\"FormAction\">");
			}
			out.print("<input type=\"hidden\" name=\"card_type_id\" value=\""
					+ toHTML(fldcard_type_id) + "\">");
...
<%
	...
	if (sForm.equals("CardTypes")) {
		sCardTypesErr = CardTypesAction(request, response, session,
				out, sAction, sForm, conn, stat);
		if ("sendRedirect".equals(sCardTypesErr))
			return;
	}
%>
<html>
	...
	<body
		...
		<table>
			<tr>
				...
				<td valign="top">
					<%
						CardTypes_Show(request, response, session, out, sCardTypesErr,
