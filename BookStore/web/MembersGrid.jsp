...
<%!void Search_Show(javax.servlet.http.HttpServletRequest request,
		...
		try {
			...
			out
					.println("     <form method=\"get\" action=\"MembersGrid.jsp\" name=\"Search\">\n     <tr>");
			...
			out
					.println("      <td style=\"background-color: #FFEAC5; border-style: inset; border-width: 0\"><font style=\"font-size: 10pt; color: #000000\">Name</font></td>");
			out
	...
	void Members_Show(javax.servlet.http.HttpServletRequest request,
		...
		if (!pname.equals("")) {
			hasParam = true;
			sWhere = "m.member_login like '%" + replace(pname, "'", "''")
					+ "%'" + " or " + "m.first_name like '%"
					+ replace(pname, "'", "''") + "%'" + " or "
					+ "m.last_name like '%" + replace(pname, "'", "''") + "%'";
		}
		if (hasParam) {
			sWhere = " WHERE (" + sWhere + ")";
		}
		...
		if (iSort == 0) {
			...
			if (sSort.equals(sSorted)) {
				...
				sForm_Sorting = "";
				sDirection = " DESC";
				sSortParams = "FormMembers_Sorting=" + sSort
						+ "&FormMembers_Sorted=" + sSort + "&";
			} else {
				sSorted = sSort;
				sForm_Sorting = sSort;
				sDirection = " ASC";
			...
			if (iSort == 2) {
				sOrder = " order by m.first_name" + sDirection;
			}
			if (iSort == 3) {
				sOrder = " order by m.last_name" + sDirection;
			}
			if (iSort == 4) {
				sOrder = " order by m.member_level" + sDirection;
			}
		}
		...
		sSQL = "select m.first_name as m_first_name, "
				...
				+ "m.member_login as m_member_login " + " from members m ";
		...
		tableHeader = "     <tr>\n      <td style=\"background-color: #FFFFFF; border-style: inset; border-width: 0\"><a href=\""
				...
				+ "?"
				+ formParams
				+ "FormMembers_Sorting=1&FormMembers_Sorted="
				+ sSorted
				+ "&\"><font style=\"font-size: 10pt; color: #CE7E00; font-weight: bold\">Login</font></a></td>\n      <td style=\"background-color: #FFFFFF; border-style: inset; border-width: 0\"><a href=\""
				+ sFileName
				+ "?"
				+ formParams
				+ "FormMembers_Sorting=2&FormMembers_Sorted="
				+ sSorted
				+ "&\"><font style=\"font-size: 10pt; color: #CE7E00; font-weight: bold\">First Name</font></a></td>\n      <td style=\"background-color: #FFFFFF; border-style: inset; border-width: 0\"><a href=\""
				+ sFileName
				+ "?"
				+ formParams
				+ "FormMembers_Sorting=3&FormMembers_Sorted="
				+ sSorted
				+ "&\"><font style=\"font-size: 10pt; color: #CE7E00; font-weight: bold\">Last Name</font></a></td>\n      <td style=\"background-color: #FFFFFF; border-style: inset; border-width: 0\"><a href=\""
				+ sFileName
				+ "?"
				+ formParams
				+ "FormMembers_Sorting=4&FormMembers_Sorted="
				+ sSorted
				+ "&\"><font style=\"font-size: 10pt; color: #CE7E00; font-weight: bold\">Level</font></a></td>\n     </tr>";
		...
		try {
			...
			while ((iCounter < RecordsPerPage) && rs.next()) {
				...
				out
						.print("      <td style=\"background-color: #FFFFFF; border-width: 1\">");
				out
						.print("<a href=\"MembersInfo.jsp?"
								+ transitParams
								+ "member_id="
								+ toURL((String) rsHash.get("m_member_id"))
				...
				out
						.print("      <td style=\"background-color: #FFFFFF; border-width: 1\">");
				fldmember_level = getValFromLOV(fldmember_level,
						"1;Member;2;Administrator");
				out.print("<font style=\"font-size: 10pt; color: #000000\">"
			...
			if (iCounter == 0) {
				...
				out
						.print("<a href=\"MembersRecord.jsp?"
								+ formParams
	...
	<body
		...
		<table>
			<tr>
				...
				<td valign="top">
					<font face=arial size=2> Enter full or partial login, first
						or last name</font>
					<%
