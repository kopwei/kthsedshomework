...
<%
	...
	if (sForm.equals("ShoppingCartRecord")) {
		sShoppingCartRecordErr = ShoppingCartRecordAction(request,
				response, session, out, sAction, sForm, conn, stat);
		if ("sendRedirect".equals(sShoppingCartRecordErr))
			return;
	}
%>

<%!String ShoppingCartRecordAction(
		...
		try {
			...
			if (iAction == iinsertAction || iAction == iupdateAction) {
				if (isEmpty(fldquantity)) {
					sShoppingCartRecordErr = sShoppingCartRecordErr
							+ "The value in field Quantity is required.<br>";
				}
				if (!isNumber(fldmember_id)) {
					sShoppingCartRecordErr = sShoppingCartRecordErr
							+ "The value in field member_id is incorrect.<br>";
				}
			...
			case iupdateAction:
				...
				sSQL = "update orders set " + "member_id="
						+ toSQL(fldmember_id, adNumber) + ",quantity="
			...
			try {
				...
				sShoppingCartRecordErr = e.toString();
				return (sShoppingCartRecordErr);
			}
	...
	void ShoppingCartRecord_Show(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
			javax.servlet.jsp.JspWriter out, String sShoppingCartRecordErr,
		...
		try {
			...
			if (bPK
					&& !(sAction.equals("insert") && "ShoppingCartRecord"
							.equals(sForm))) {
			...
			// Set lookup fields
			flditem_id = dLookUp(stat, "items", "name", "item_id="
			...
			out
					.print("<input type=\"text\"  name=\"quantity\" maxlength=\"5\" value=\""
							+ toHTML(fldquantity) + "\" size=\"5\">");
			...
			if (bPK
					&& !(sAction.equals("insert") && "ShoppingCartRecord"
							.equals(sForm))) {
				out
						.print("<input type=\"submit\" value=\"Update\" onclick=\"document.ShoppingCartRecord.FormAction.value = 'update';\">");
				out
						.print("<input type=\"submit\" value=\"Delete\" onclick=\"document.ShoppingCartRecord.FormAction.value = 'delete';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.ShoppingCartRecord.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"ShoppingCartRecord\"><input type=\"hidden\" value=\"update\" name=\"FormAction\">");
			}
				...
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.ShoppingCartRecord.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"ShoppingCartRecord\"><input type=\"hidden\" value=\"insert\" name=\"FormAction\">");
			}
	...
	<body
		...
		<table>
			<tr>
				...
				<td valign="top">
					<%
						ShoppingCartRecord_Show(request, response, session, out,
								sShoppingCartRecordErr, sForm, sAction, conn, stat);
					%>
