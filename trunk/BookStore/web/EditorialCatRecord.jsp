...
<%!String editorial_categoriesAction(
		...
		try {
			...
			if (iAction == iupdateAction || iAction == ideleteAction) {
				...
				if (isEmpty(pPKeditorial_cat_id))
					return seditorial_categoriesErr;
				sWhere = "editorial_cat_id="
						+ toSQL(pPKeditorial_cat_id, adNumber);
			}
			...
			case iinsertAction:
				...
				sSQL = "insert into editorial_categories ("
						...
						" values (" + toSQL(fldeditorial_cat_name, adText)
						+ ")";
				break;
			...
			try {
				...
				seditorial_categoriesErr = e.toString();
				return (seditorial_categoriesErr);
			}
	...
	void editorial_categories_Show(
		...
		try {
			...
			if (isEmpty(peditorial_cat_id)) {
				bPK = false;
			}

			sWhere += "editorial_cat_id=" + toSQL(peditorial_cat_id, adNumber);
			primaryKeyParams += "<input type=\"hidden\" name=\"PK_editorial_cat_id\" value=\""
					+ peditorial_cat_id + "\"/>";
			...
			if (bPK
					&& !(sAction.equals("insert") && "editorial_categories"
							.equals(sForm))) {
				...
				if ("".equals(seditorial_categoriesErr)) {
					...
					fldeditorial_cat_name = (String) rsHash
							.get("editorial_cat_name");
				}
				...
				if ("".equals(seditorial_categoriesErr)) {
					fldeditorial_cat_id = toHTML(getParam(request,
							"editorial_cat_id"));
				}
			}
			...
			out
					.print("<input type=\"text\"  name=\"editorial_cat_name\" maxlength=\"50\" value=\""
							+ toHTML(fldeditorial_cat_name) + "\" size=\"50\">");
			...
			if (bPK
					&& !(sAction.equals("insert") && "editorial_categories"
							.equals(sForm))) {
				out
						.print("<input type=\"submit\" value=\"Update\" onclick=\"document.editorial_categories.FormAction.value = 'update';\">");
				out
						.print("<input type=\"submit\" value=\"Delete\" onclick=\"document.editorial_categories.FormAction.value = 'delete';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.editorial_categories.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"editorial_categories\"><input type=\"hidden\" value=\"update\" name=\"FormAction\">");
			}

			else {
				out
						.print("<input type=\"submit\" value=\"Insert\" onclick=\"document.editorial_categories.FormAction.value = 'insert';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.editorial_categories.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"editorial_categories\"><input type=\"hidden\" value=\"insert\" name=\"FormAction\">");
			}
			out
					.print("<input type=\"hidden\" name=\"editorial_cat_id\" value=\""
...
<%
	...
	if (sForm.equals("editorial_categories")) {
		seditorial_categoriesErr = editorial_categoriesAction(request,
				response, session, out, sAction, sForm, conn, stat);
		if ("sendRedirect".equals(seditorial_categoriesErr))
			return;
	}
%>
<html>
