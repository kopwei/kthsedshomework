...
<%
	...
	if (sForm.equals("editorials")) {
		seditorialsErr = editorialsAction(request, response, session,
				out, sAction, sForm, conn, stat);
		if ("sendRedirect".equals(seditorialsErr))
			return;
	}
%>

<%!String editorialsAction(javax.servlet.http.HttpServletRequest request,
		...
		try {
			...
			if (iAction == iinsertAction || iAction == iupdateAction) {
				if (isEmpty(fldeditorial_cat_id)) {
					seditorialsErr = seditorialsErr
							+ "The value in field Editorial Category is required.<br>";
				}
				if (!isNumber(fldeditorial_cat_id)) {
					seditorialsErr = seditorialsErr
							+ "The value in field Editorial Category is incorrect.<br>";
				}
			...
			case iinsertAction:
				...
				sSQL = "insert into editorials (" + "article_desc,"
						+ "article_title," + "editorial_cat_id," + "item_id)" +

						" values (" + toSQL(fldarticle_desc, adText) + ","
			...
			case iupdateAction:
				...
				sSQL = "update editorials set " + "article_desc="
						+ toSQL(fldarticle_desc, adText) + ",article_title="
						+ toSQL(fldarticle_title, adText)
						+ ",editorial_cat_id="
						+ toSQL(fldeditorial_cat_id, adNumber) + ",item_id="
						+ toSQL(flditem_id, adNumber);
	...
	void editorials_Show(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
			javax.servlet.jsp.JspWriter out, String seditorialsErr,
		...
		try {
			...
			primaryKeyParams += "<input type=\"hidden\" name=\"PK_article_id\" value=\""
					+ particle_id + "\"/>";
			...
			if (bPK
					&& !(sAction.equals("insert") && "editorials".equals(sForm))) {
				...
				if ("".equals(seditorialsErr)) {
					...
					fldeditorial_cat_id = (String) rsHash
							.get("editorial_cat_id");
			...
			out
					.print("     <tr>\n      <td style=\"background-color: #FFEAC5; border-style: inset; border-width: 0\"><font style=\"font-size: 10pt; color: #000000\">Article Description</font></td><td style=\"background-color: #FFFFFF; border-width: 1\">");
			out
					.print("<input type=\"text\"  name=\"article_desc\" maxlength=\"\" value=\""
							+ toHTML(fldarticle_desc) + "\" size=\"\">");

			out.println("</td>\n     </tr>");

			out
					.print("     <tr>\n      <td style=\"background-color: #FFEAC5; border-style: inset; border-width: 0\"><font style=\"font-size: 10pt; color: #000000\">Article Title</font></td><td style=\"background-color: #FFFFFF; border-width: 1\">");
			out
					.print("<input type=\"text\"  name=\"article_title\" maxlength=\"200\" value=\""
							+ toHTML(fldarticle_title) + "\" size=\"50\">");
			...
			out
					.print("<select name=\"editorial_cat_id\">"
							+ getOptions(
									conn,
									"select editorial_cat_id, editorial_cat_name from editorial_categories order by 2",
									false, true, fldeditorial_cat_id)
							+ "</select>");
			...
			if (bPK
					&& !(sAction.equals("insert") && "editorials".equals(sForm))) {
				out
						.print("<input type=\"submit\" value=\"Update\" onclick=\"document.editorials.FormAction.value = 'update';\">");
				out
						.print("<input type=\"submit\" value=\"Delete\" onclick=\"document.editorials.FormAction.value = 'delete';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.editorials.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"editorials\"><input type=\"hidden\" value=\"update\" name=\"FormAction\">");
			}

			else {
				out
						.print("<input type=\"submit\" value=\"Insert\" onclick=\"document.editorials.FormAction.value = 'insert';\">");
				out
						.print("<input type=\"submit\" value=\"Cancel\" onclick=\"document.editorials.FormAction.value = 'cancel';\">");
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"editorials\"><input type=\"hidden\" value=\"insert\" name=\"FormAction\">");
			}
			out.print("<input type=\"hidden\" name=\"article_id\" value=\""
					+ toHTML(fldarticle_id) + "\">");
	...
	<body
		...
		<table>
			<tr>
				...
				<td valign="top">
					<%
						editorials_Show(request, response, session, out, seditorialsErr,
