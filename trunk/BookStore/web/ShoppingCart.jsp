...
<%
	...
	if (sForm.equals("Member")) {
		sMemberErr = MemberAction(request, response, session, out,
				sAction, sForm, conn, stat);
		if ("sendRedirect".equals(sMemberErr))
			return;
	}
%>
<%
	...
	if (conn != null)
		conn.close();
%>
<%!void Items_Show(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
			javax.servlet.jsp.JspWriter out, String sItemsErr, String sForm,
		...
		try {
			...
			while (rs.next()) {
				...
				out
						.print("<a href=\"ShoppingCartRecord.jsp?"
								+ transitParams
								+ "order_id="
								+ toURL((String) rsHash.get("order_id"))
								+ "&\"><font style=\"font-size: 10pt; color: #000000\">"
								+ toHTML(fldField1) + "</font></a>");
	...
	String MemberAction(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
	...
	void Member_Show(javax.servlet.http.HttpServletRequest request,
			javax.servlet.http.HttpServletResponse response,
			javax.servlet.http.HttpSession session,
			javax.servlet.jsp.JspWriter out, String sMemberErr, String sForm,
			String sAction, java.sql.Connection conn, java.sql.Statement stat)
			throws java.io.IOException {
		try {
			...
			out.print("<a href=\"MyInfo.jsp?" + transitParams
					+ "\"><font style=\"font-size: 10pt; color: #000000\">"
			...
			if (bPK && !(sAction.equals("insert") && "Member".equals(sForm))) {
				...
				out
						.print("<input type=\"hidden\" name=\"FormName\" value=\"Member\"><input type=\"hidden\" value=\"\" name=\"FormAction\">");
			}
	...
	<body
		...
		<table>
			<tr>
				...
				<td valign="top">
					<%
						Member_Show(request, response, session, out, sMemberErr, sForm,
								sAction, conn, stat);
					%>
		...
		<table>
			<tr>
				<td valign="top">
					<%
						Items_Show(request, response, session, out, sItemsErr, sForm,
								sAction, conn, stat);
					%>
