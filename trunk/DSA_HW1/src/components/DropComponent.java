...
public class DropComponent {
	...
	public void handleMessageEvent(MessageEvent event) {
		...
		log.info("Link to " + event.getDestination().getId()
				+ " has loss rate of: " + linkDescriptor.getLossRate()
				+ " msg=" + event);
		...
		if (randNum <= 1.0 - linkDescriptor.getLossRate()) {
			...
			log.info("Message " + event.getClass().getSimpleName() + " to "
					+ event.getDestination().getId() + " dropped");
		}
	}
}
